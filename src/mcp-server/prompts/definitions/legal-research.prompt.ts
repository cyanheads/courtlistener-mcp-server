...
Rate limit note: CourtListener publishes free-tier limits of 5 req/min, 50/hr, 125/day, though actual limits vary by token tier. The server queues its own requests to the minute and hour windows, so a burst waits rather than failing; if a call still comes back rate-limited, honor its Retry-After.`,
        },
      },
    ];
  },
